...
public class WeixinMenuentityManageImpl extends HibernateEntityDao<WeixinMenuentity>{
	 ...
	 * @return
	 */
	public String getListSelect(WeixinMenuentity productCategory,String indent,WeixinMenuentity curent){
		...
		if (null!=productCategory) {
			...
			List<WeixinMenuentity> children = find("from WeixinMenuentity where  weixinMenuentity.dbid=? order by orders",productCategory.getDbid());
	 ...
	 * @return
	 */
	public String getTableTr(WeixinMenuentity productCategory,String indent){
		StringBuilder sb = new StringBuilder();
		if (null!=productCategory) {
			List<WeixinMenuentity> children = find("from WeixinMenuentity where  weixinMenuentity.dbid=? order by orders",productCategory.getDbid());
			if (null!=children&&children.size()>0) {
				for (WeixinMenuentity productCateGorup2 : children) {
					...
					sb.append("<a href='javascript:void(-1)' onclick=\"window.location.href='/weixinMenuentity/edit?dbid="+productCateGorup2.getDbid()+"&parentMenu=1'\">编辑</a> |"); 
					sb.append("<a href='javascript:void(-1)' onclick=\"$.utile.deleteById('/weixinMenuentity/delete?dbid="+productCateGorup2.getDbid()+"','searchPageForm')\" title='删除'>删除</a></td>");
					sb.append("</tr>");
					List<WeixinMenuentity> findBy = find("from WeixinMenuentity where  weixinMenuentity.dbid=? order by orders",productCateGorup2.getDbid());
					if (null!=findBy&&findBy.size()>0) {
	...
	public String getProductCateGorySelect(Integer businessId,WeixinMenuentity curent) {
		List<WeixinMenuentity> productCategories=find("from WeixinMenuentity where accountid=? and weixinMenuentity.dbid IS NULL", new Object[]{businessId+""});
		String select="";
