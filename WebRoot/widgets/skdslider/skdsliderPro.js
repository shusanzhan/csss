...
(function($){
  ...
  $.skdslider = function(container,options){
		...
		if(config.animationType=='sliding'){
		   ...
		   slides.css({'width':element.outerWidth()-20,'height':element.outerHeight()});
  ...
  $.skdslider.createNav=function(element,slides,config){
			...
			if (config.showNav==true){
					...
					var length=element.find('.slide-navs').children("li").length;
					//nav_width=parseInt((nav_width/2));
					nav_width=(-1)*nav_width*length+(-5);
  ...
  $.skdslider.next=function(element,slides,config){
    if((config.currentSlide+1)==slides.length)targetSlide = slides.length-1;
	else targetSlide = (config.currentSlide+1);
  ...
  $.skdslider.prev=function(element,slides,config){
    if(config.currentSlide==0)targetSlide = 0;
	else targetSlide = (config.currentSlide-1);
